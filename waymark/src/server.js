...
function createServer(db = openDb()) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && pathname.match(/^\/api\/books\/[A-Za-z0-9_-]+$/)) {
        const token = pathname.split('/')[3];
        try {
          return json(res, 200, stamps.getSharedBook(db, token), req);
        } catch (e) {
          return json(res, e.status || 400, security.clientErrorPayload(e, 'Book unavailable'), req);
        }
      }
      ...
      if (req.method === 'GET' && pathname === '/api/stamps') {
        return json(res, 200, stamps.listStamps(db, user.id), req);
      }
      if (req.method === 'GET' && pathname === '/api/stats') {
        return json(res, 200, stamps.stats(db, user.id), req);
      }
      if (req.method === 'POST' && pathname === '/api/stamps') {
        const body = await security.parseBodyLimited(req);
        try {
          return json(res, 201, await stamps.dropStamp(db, user, body, req), req);
        } catch (e) {
          const status = e.status || 400;
          return json(res, status, security.clientErrorPayload(e, 'Could not save stamp'), req);
        }
      }
      if (req.method === 'GET' && pathname.match(/^\/api\/stamps\/\d+$/)) {
        const stampId = Number(pathname.split('/')[3]);
        try {
          return json(res, 200, stamps.getStamp(db, user.id, stampId), req);
        } catch (e) {
          return json(res, e.status || 400, security.clientErrorPayload(e, 'Stamp unavailable'), req);
        }
      }
      if (req.method === 'DELETE' && pathname.match(/^\/api\/stamps\/\d+$/)) {
        const stampId = Number(pathname.split('/')[3]);
        try {
          return json(res, 200, stamps.deleteStamp(db, user, stampId, req), req);
        } catch (e) {
          return json(res, e.status || 400, security.clientErrorPayload(e, 'Could not delete stamp'), req);
        }
      }
      if (req.method === 'POST' && pathname === '/api/share') {
        return json(res, 201, stamps.createShareLink(db, user, req), req);
      }
      if (req.method === 'POST' && pathname === '/api/share/revoke') {
        return json(res, 200, stamps.revokeShareLinks(db, user, req), req);
      }
      ...
      if (status >= 500) console.error('[waymark]', err && err.stack ? err.stack : err);
...
if (require.main === module) {
  ...
  server.listen(PORT, bindHost, () => {
    console.log(`Waymark listening on http://${bindHost}:${PORT}`);
