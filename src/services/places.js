/**
 * Places: personal location pins + same-place chat.
 * Coordinates are stored as integer microdegrees (degrees * 1e6). Place matching
 * uses a 100m radius so people who were at the same spot share a room.
...
const PUBLIC_QUANTUM_MICRO = 1_000; // ~111m
const MAX_LABEL = 80;
const MAX_NOTE = 280;
...
/** Named check-in spots so colleagues can meet in the same room without sharing GPS. */
const LANDMARKS = [
...
function defaultLabel(latMicro, lngMicro) {
  const lat = publicDegrees(latMicro).toFixed(3);
  const lng = publicDegrees(lngMicro).toFixed(3);
  return `Pin ${lat}, ${lng}`;
}
...
function findNearbyPlace(db, latMicro, lngMicro) {
  ...
  const rows = db.prepare(`
    ...
    FROM places
    WHERE lat_micro BETWEEN ? AND ?
      AND lng_micro BETWEEN ? AND ?
...
function visitorCount(db, placeId) {
  return db.prepare(
    'SELECT COUNT(DISTINCT user_id) AS n FROM place_visits WHERE place_id = ?'
  ).get(placeId).n;
}
...
function listMyPlaces(db, userId) {
  ...
  return {
    ...
    myPins: pins.map(serializeOwnPin),
  };
}
...
function dropPin(db, user, body, req = null) {
  ...
  if (!limit.ok) {
    throw fail('Too many pins. Try again later.', 429, 'rate_limited');
  }
  ...
  try {
    ...
    if (nearby) {
      ...
      const ins = db.prepare(`
        INSERT INTO places(label, lat_micro, lng_micro, created_by)
        VALUES (?, ?, ?, ?)
      `).run(label || defaultLabel(latMicro, lngMicro), latMicro, lngMicro, user.id);
      placeId = Number(ins.lastInsertRowid);
    }
    db.prepare(`
      INSERT INTO place_visits(place_id, user_id, lat_micro, lng_micro, note)
      VALUES (?, ?, ?, ?, ?)
...
function deletePin(db, user, pinId, req = null) {
  const row = db.prepare(
    'SELECT id, place_id, user_id FROM place_visits WHERE id = ?'
  ).get(pinId);
  if (!row) throw fail('Pin not found', 404, 'not_found');
  if (row.user_id !== user.id) throw fail('You can only remove your own pins', 403, 'FORBIDDEN');
  db.prepare('DELETE FROM place_visits WHERE id = ?').run(pinId);
  audit(db, {
    actorId: user.id,
    action: 'place.pin_delete',
    entityType: 'place',
    entityId: row.place_id,
    detail: { pinId: row.id },
    req,
  });
  const still = hasVisited(db, user.id, row.place_id);
  return { ok: true, placeId: row.place_id, stillMember: still };
}
...
function postMessage(db, user, placeId, body, req = null) {
  ...
  if (!limit.ok) {
    throw fail('Too many messages. Try again later.', 429, 'rate_limited');
  }
...
function seedDemo(db) {
  ...
  const avery = byEmail('avery@firm.example');
  const jordan = byEmail('jordan@firm.example');
  const riley = byEmail('riley@firm.example');
  const sam = byEmail('sam@firm.example');
  if (!avery || !jordan) return { seeded: false };

  const existing = db.prepare('SELECT COUNT(*) AS n FROM places').get().n;
  if (existing > 0) return { seeded: false };

  const actors = [
    { user: avery, landmark: 'ferry-building', note: 'Hearing wrap-up' },
    { user: jordan, landmark: 'ferry-building', note: 'Coffee after court' },
    { user: sam, landmark: 'ferry-building', note: 'Dropped exhibits' },
    { user: riley, landmark: 'civic-center', note: 'Filing window' },
    { user: avery, landmark: 'civic-center', note: 'Status conference' },
  ...
  if (ferry) {
    ...
    db.prepare('INSERT INTO place_messages(place_id, user_id, body) VALUES (?, ?, ?)')
      .run(ferry.id, avery.id, 'Just pinned — grabbing a table if you want to debrief.');
    if (sam) {
      db.prepare('INSERT INTO place_messages(place_id, user_id, body) VALUES (?, ?, ?)')
        .run(ferry.id, sam.id, 'On my way from the drop-off. Two minutes.');
    }
  }
  if (civic && riley) {
    db.prepare('INSERT INTO place_messages(place_id, user_id, body) VALUES (?, ?, ?)')
      .run(civic.id, riley.id, 'Line at Civic Center is short this morning.');
  }
  return { seeded: true };
}
