 ...
 *
 * `npm run seed` resets the database then seeds.
 * `applyDemoSeed(db)` inserts demo data only when the users table is empty
 * (used by non-production server boot so sign-in works on a fresh DB).
 */
...
function defaultDemoPassword() {
  return process.env.DEMO_PASSWORD || 'demo-change-me';
}

/**
 * Insert demo users and related sample data when no users exist.
 * Never wipes existing rows.
 */
function applyDemoSeed(db, { password } = {}) {
  const userCount = db.prepare('SELECT COUNT(*) AS n FROM users').get().n;
  if (userCount > 0) {
    return { seeded: false, userCount };
  }

  const demoPassword = password || defaultDemoPassword();
...
function seedDatabase(dbFile = process.env.DB_FILE || DEFAULT_DB) {
  const db = resetDb(dbFile);
  const result = applyDemoSeed(db);
  return { db, dbFile, ...result };
}

if (require.main === module) {
  const { dbFile, password } = seedDatabase();
  const demoPassword = password || defaultDemoPassword();
